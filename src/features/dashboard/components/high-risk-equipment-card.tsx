  ...
  return (
    <div className="rounded-[20px] border border-slate-200 bg-white shadow-sm sm:rounded-[24px] dark:border-white/10 dark:bg-[#171815]">
      <div className="flex items-center justify-between border-b border-slate-100 px-4 py-4 sm:px-6 sm:py-5 dark:border-white/8">
        <div>
          <h2 className="text-base font-semibold text-slate-900 dark:text-stone-100">High-risk equipment</h2>
            ...
            <div key={item.id} className="px-4 py-3 sm:px-6">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <div className="min-w-0">
                  <p className="truncate text-sm font-semibold text-slate-900 dark:text-stone-100">{item.asset}</p>
                  <p className="mt-0.5 text-xs text-slate-500 dark:text-stone-400">{item.site}</p>
                </div>
                <span className={`rounded-full px-2.5 py-0.5 text-xs font-semibold ${riskColor[item.riskLevel] ?? riskColor.High}`}>
                  {item.riskLevel} {item.riskScore}%
                </span>
              </div>
              <p className="mt-2 text-xs leading-5 text-slate-500 dark:text-stone-400">{item.recommendation}</p>
            </div>
          ))}
        </div>
      )}
