...
import {
  ...
  type OpenMaintenanceSchemaValues,
...
export function OpenMaintenanceModal({
  ...
  const {
    ...
    setError,
    reset,
  } = useForm<OpenMaintenanceSchemaValues>({
  ...
  }, [open, reset, technicians]);

  async function onSubmit(values: OpenMaintenanceSchemaValues) {
    const result = await sileo
      .promise(
        async () => {
          const submission = await run(
            () => maintenanceService.openFromRequest({
        ...
        {
          loading: {
            title: "Opening maintenance...",
            description: `Converting ${requestTicket} into a work order.`,
          },
          success: (response) => ({
          ...
          error: (errorValue) => ({
            title: "Open maintenance failed",
            description:
              typeof errorValue === "object" && errorValue !== null && "message" in errorValue
                ? String((errorValue as { message?: unknown }).message)
                : "The maintenance work order could not be opened.",
          }),
        },
      )
      .catch((errorValue: ApiErrorShape) => {
        if (errorValue.fieldErrors) {
          Object.entries(errorValue.fieldErrors).forEach(([field, message]) => {
            setError(field as keyof OpenMaintenanceSchemaValues, {
              type: "server",
              message,
            });
          });
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-slate-400 dark:text-stone-500">
                    Convert {requestTicket} into a work order.
                  </p>
                </div>
              </div>
              <button
              ...
              <form className="space-y-4" onSubmit={handleSubmit(onSubmit)}>
                ...
                <div className="flex justify-end gap-3 pt-2">
                  ...
                  >
                    ...
                    {isSubmitting ? "Opening..." : "Open maintenance"}
                  </button>
                </div>
              </form>
            </div>
