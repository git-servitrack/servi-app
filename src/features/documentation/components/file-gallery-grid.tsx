...
import { Camera, ChevronLeft, ChevronRight, LayoutGrid, Table2 } from "lucide-react";
...
export function FileGalleryGrid({ files }: { files: DocumentationFile[] }) {
  ...
  function openPreview(file: DocumentationFile) {
    setPreviewFile(file);
    setPreviewOpen(true);
  }
    ...
    <div className="rounded-[20px] border border-slate-200 bg-white p-4 shadow-sm sm:rounded-[24px] sm:p-6 dark:border-white/10 dark:bg-[#171815]">
      ...
      {filtered.length === 0 ? (
        ...
        <div className="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
          {paginated.map((file) => (
            <MediaPreviewCard key={file.id} file={file} onPreview={() => openPreview(file)} />
          ))}
        </div>
      ) : (
                  ...
                  <tr key={file.id} className="transition-colors hover:bg-slate-50/80 dark:hover:bg-white/4">
                    <td className="px-4 py-3">
                      <div className="flex h-12 w-16 items-center justify-center rounded-lg bg-linear-to-br from-slate-100 to-[#145d66]/10 dark:from-white/8 dark:to-[#145d66]/15">
                        <Camera className="h-5 w-5 text-[#145d66] dark:text-[#86d0d8]" />
                      </div>
                    </td>
      ...
      <DocumentationPreviewModal file={previewFile} open={previewOpen} onClose={closePreview} />
    </div>
  );
}
